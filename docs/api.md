...
```json
{
  "version": "2.3.0-beta-575b61b-dirty"
}
```
...
- `satPerVbyte`: if the prepay miner fee protocol is enabled, a custom miner fee in satoshi per vbyte for the lockup transaction can be specified
...
- `lockupAddress`:  add derived from the `redeemScript` to which Boltz will lockup coins
